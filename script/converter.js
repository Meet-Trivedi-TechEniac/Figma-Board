...
//         // Apply leave colors for Leave Conflicts tab
//         resetFilters();
//     });
// }
...
function renderEventDetails(arg) {
    ...
    return {
        html: `
        <div class='event-disp-container ${idToClass[arg.event.id]}'
        data-bs-toggle="tooltip"
        data-bs-html="true"
...
function refreshCalendarUI() {
    ...
    syncDynamicHeight();
    applyObserver();
    console.log("after Update The Evemts are", eventData)
}
...
function getTerritory() {
    ...
    //     "territory",
    //     "?$select=territoryid,name&$filter=statecode eq 0"
    // );
}
        ...
        resolve({
            entities: [
                ...
                },
                {
                    ...
                    "msdyn_name": "00313",
                    "msdyn_agreementbookingdateid": "b3141cf1-91e1-ee11-904c-00sd0d3aca6924",
...
function mapOverLeaveData(response) {
    ...
    const mappedEvents = response.entities.map((r) => {
        const resourceId = r?._msdyn_resource_value;
        const leaveType = r?.leavetype;
        const className = leaveTypeClassMap[leaveType] ?? "ec-event-default";

        if (resourceId) {
            idToClass[resourceId] = className;
        }
